...
The `home-routes` handler in the `raid-analyzer.routes.home` namespace
...
The `home-page` function will in turn call the `raid-analyzer.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `raid-analyzer.handler` namespace:
...
Request middleware functions are located under the `raid-analyzer.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `raid-analyzer.dev-middleware` namespace found in
